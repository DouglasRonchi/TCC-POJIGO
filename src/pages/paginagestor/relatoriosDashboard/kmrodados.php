...
<html lang="en">
<head>
    <title>Relatório <?= $_GET['ano'] ?></title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
                ...
                <div class="col-md-12">
                    <div id="printableArea" class="conteudo m-3">
                        <div class="text-center">
                            <h3>Relatório de Km's rodados</h3>
                        </div>
                        ...
                        <div class="mt-3">
                            <table class="table">
                                ...
                                <?php
                                $query = $conn->executeQuery("SELECT SUM(rp.quilometragem) as quilometragem,MONTH(rp.hora_inicio) FROM registro_ponto rp WHERE YEAR(rp.hora_inicio)={$_GET['ano']} GROUP BY MONTH(rp.hora_inicio)");
                                $rows = mysqli_num_rows($query);
                                while ($rows = mysqli_fetch_assoc($query)):
                                    ?>
                                    <tr>
                                        <td><?= $rows['MONTH(rp.hora_inicio)'] ?></td>
                                        <td><?= $rows['quilometragem'] ?></td>
                                    </tr>
                                <?php
                                endwhile;
                                ?>
